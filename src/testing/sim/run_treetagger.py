...
#output_path = "/mnt/nfs/scratch1/hschang/language_modeling/NSD_for_sentence_embedding/dataset_testing/STS/stsbenchmark/sts-dev_org_lemma"
#tokenized = False
...
output_path = "/mnt/nfs/scratch1/hschang/language_modeling/NSD_for_sentence_embedding/dataset_testing/WiC_dataset/dev/dev.data_org_lemma"
tokenized = True
...
def append_w_ind(w_lemma, lemma_sent):
    if w_lemma != '<unknown>':
        opt_ind = w_lemma.find('|')
        if opt_ind>0:
            lemma_sent.append(w_lemma[:opt_ind])
        else:
            lemma_sent.append(w_lemma)
...
for i_out, sent in enumerate(input_sent):
    ...
    #print(sent)
    for word_num, w in enumerate(sent.split()):
        #try: 
        ...
        if w != tagged_results[w_ind][0].replace('\\',''): #handle the case where the tagger split the token such as MD.
            ...
            if tokenized:
                ...
                append_w_ind(tagged_results[w_ind][2], lemma_sent)
            #print(w)
            #print(sent)
            #print(tagged_results[w_ind])
        ...
        if tagged_results[w_ind][2] != '<unknown>':
            if str.isupper(w[0]) and word_num != 0 and tagged_results[w_ind-1][1] != 'SENT' and tagged_results[w_ind-1][1] != 'POS':
                lemma_sent.append(w)
            else:
