...
OUTPUT_DIR="/iesl/canvas/hschang/language_modeling/NSD_for_sentence_embedding/data/raw/wiki2016/"

for file_path in $INPUT_DIR/*; do
    file_name=`basename $file_path`
    output_file_name="${file_name//.json.gz/}"
    echo --ntasks=1 --nodes=1 --exclusive -p cpu ~/anaconda3/bin/python src/preprocessing/tokenize_wiki.py -i $file_path -o $OUTPUT_DIR/$output_file_name
    srun --ntasks=1 --nodes=1 --exclusive -p cpu ~/anaconda3/bin/python src/preprocessing/tokenize_wiki.py -i $file_path -o $OUTPUT_DIR/$output_file_name  &
