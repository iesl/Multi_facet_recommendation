...
if [ "$#" -eq 6 ]; then
    ...
    #INPUT_NAME=NeurIPS2020_fixed_review
    #INPUT_NAME=NeurIPS2020_fixed_ac
    #INPUT_NAME=NeurIPS2020_final_review
    #INPUT_NAME=NeurIPS2020_final
    #INPUT_NAME=NeurIPS2020
    #INPUT_NAME=ICLR2018
    #INPUT_NAME=ICLR2019
    INPUT_NAME=ICLR2020
    ...
    INPUT_FILE_TEST_ALL="$TEXT_DATA_DIR/all_submission_paper_data"
...
if [ -f $INPUT_FILE_TEST_ALL ]; then
    eval $PY_PATH src/preprocessing/Amazon/amazon_split_files.py -i $INPUT_FILE_TEST_ALL -f ${INPUT_FILE}_test -y $OUTPUT_DIR_TYPE/corpus_index_test -u ${INPUT_FILE_USER}_test -t ${INPUT_FILE_TAG}_test -p ${OUTPUT_DIR}/paper_id_test
fi
...
if [ -f $INPUT_FILE_TEST_ALL ]; then
    eval $PY_PATH src/preprocessing/map_indices_to_tensors.py --input_file_name corpus_index_test --data_feature $OUTPUT_DIR_FEATURE --data_type $OUTPUT_DIR_TYPE --data_user $OUTPUT_DIR_USER --data_tag $OUTPUT_DIR_TAG --save $OUTPUT_DIR/$TENSOR_FOLDER/val.pt --max_sent_len $MAX_SENT_LEN --cv_fold_num 0
    cp $OUTPUT_DIR/$TENSOR_FOLDER/val.pt $OUTPUT_DIR/$TENSOR_FOLDER/test.pt
else
    cp $OUTPUT_DIR/$TENSOR_FOLDER/train.pt $OUTPUT_DIR/$TENSOR_FOLDER/val.pt
    echo "Warning: No testing is found."
fi
